...
- (void)hook_OnSyncBatchAddMsgs:(NSArray *)msgs isFirstSync:(BOOL)arg2 {
    ...
    /*
     ...
     NSString *message = addMsg.content.string;
     
     NSLog(@"收到一条消息");
     NSLog(@"来自[%@]发的 内容是【%@】", addMsg.fromUserName.string, message);
     NSString *currentUserName = [objc_getClass("CUtility") GetCurrentUserName];
     
     NSString * toUsrName = addMsg.toUserName.string;
     NSString * fromUsrName = addMsg.fromUserName.string;
     // 是否艾特了自己
     if ([self isWasAt:addMsg.msgSource curUserName:currentUserName]) {
     
     GroupStorage *contactStorage = [[objc_getClass("MMServiceCenter") defaultCenter] getService:objc_getClass("GroupStorage")];
     WCContactData *fromContact = [contactStorage GetGroupMemberContact:toUsrName];
     NSString * inGroupNickName = [fromContact getGroupChatNickNameInGroup:fromUsrName];
     NSLog(inGroupNickName);
     NSString * formatMessage = message;
     ...
     [Networking.shaerd chat:formatMessage succesBlock:^(id response) {
     [self handleGPTResponse:response addMsg:addMsg];
     ...
     // 请求chat gpt
     [self replySelfWithMsg:@"系统提示:ChatGPT大脑正在飞速运转,请稍等..." addMsg:addMsg];
     NSString *receivedText = addMsg.content.string;
     [Networking.shaerd chat:receivedText succesBlock:^(id response) {
     [self handleGPTResponse:response addMsg:addMsg];
     } failedBlock:^(NSError *error) {
